...
function preview(items) {
  ...
  items.forEach(function (item, index) {
    ...
    if (item.thumb_url) {
      carouselItem.find('.carousel-images').css('background-image', 'url(\'' + item.url + '?timestamp=' + item.time + '\')');
    } else {
      carouselItem.find('.carousel-images').css('width', '50vh').append($('<div>').addClass('mime-icon ico-' + item.icon));
    }
